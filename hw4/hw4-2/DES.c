...
int DES_key_expansion(unsigned char* subkey, unsigned char* key) {
    int table[16] = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};
    unsigned char buf[56], tmp[56];
    int i;
    for(i = 0; i < 6; i ++)
        byte_to_bit(buf + i * 8, key + i);
    PC1(buf, buf);
    for(i = 0; i < 16; i ++) {
...
int DES(unsigned char* block, unsigned char* subkey, unsigned char* result) {
